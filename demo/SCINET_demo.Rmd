...
Bcell.cols = which(sce$CellType == "B cell")
...
sce = readRDS('../../input/datasets/Ding2019/HumanPBMC.RDS')
...
# thread_no = 8
# sce = reduce.sce(sce)
# ACTIONet.out = run.ACTIONet(sce, k_max = depth, thread_no = thread_no)
# 
# save(sce, ACTIONet.out, file = 'Ding_PBMC.RData')

load('Ding_PBMC.RData')

W = ACTIONet.out$signature.profile[, ACTIONet.out$core.out]
...
activity.scores.ACTIONet = compute_gene_activities_decoupled(archetypes = ACTIONet.out$signature.profile, H = ACTIONet.out$reconstruct.out$H_stacked , samples = Bcell.cols, thread_no = 8)
